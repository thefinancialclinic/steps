...
it("displays a NavLink for each global link", () => {
    const dropdown = shallow(
        <NavDropdown title='Some title' />
    );
    const globalLinks = dropdown.find('.global-links').shallow().find('NavLink');
...
it("displays a NavLink for each contextual link", () => {
    ...
    const contextualLinks = dropdown.find('.contextual-links').shallow().find('NavLink');
